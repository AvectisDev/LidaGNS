...
# Соответствие «номер считывателя Лиды → метод Мириады» (описание ролей — в services/rfid.py):
#   1, 2  Погрузка полного баллона на трал        → /balloontocar   (партия отгрузки, batch_type='u')
#   3, 4  Приёмка пустого баллона из трала        → /balloontosklad (партия приёмки, batch_type='l')
#   5     Регистрация полного баллона на складе   → /balloontosklad (fulness=1, партии нет)
#   7, 8  Наполнение баллона. Карусели №1 и №2    → /fillingballoon
# Ридеры 6 и 9–12 обслуживают перемещения внутри цехов и статусы в Мириаду не отправляют.
LOADING_INTO_TRUCK_READERS = frozenset({1, 2})
BATCH_WAREHOUSE_READERS = frozenset({3, 4})
FULL_WAREHOUSE_READERS = frozenset({5})
FILLING_READERS = frozenset({7, 8})
...
def get_balloon_data_from_miriada(nfc_tag: str) -> Optional[Dict[str, Any]]:
    ...
    url = (
        f'{settings.MIRIADA_API_URL}/getballoonbynfctag'
        f'?nfctag={nfc_tag}&realm={settings.MIRIADA_GET_REALM}'
    )
...
def _prepare_payload_for_miriada(
    ...
    Args:
        reader (int): номер RFID-считывателя (1/2/3/4/5/7/8).
        nfc_tag (str): NFC-метка баллона.
    ...
    payload = {
        'nfctag': nfc_tag,
        'realm': settings.MIRIADA_POST_REALM
    }

    if reader in FILLING_READERS:
        send_type = 'filling'
    elif reader in BATCH_WAREHOUSE_READERS:
    ...
    elif reader in FULL_WAREHOUSE_READERS:
        # Полный баллон выходит из наполнительного цеха вне партии — как в прежнем обработчике.
        send_type = 'registering_in_warehouse'
        payload.update({'fulness': 1})
    elif reader in LOADING_INTO_TRUCK_READERS:
