...
# Роли считывателей Лиды описаны в filling_station/services/rfid.py.
# Рамки отгрузки (1, 2) и приёмки (3, 4) работают с партиями: статусы их баллонов
# уходят в Мириаду одним пакетом при закрытии партии, а не в момент сканирования.
MIRIADA_BATCH_STATUS_READERS = frozenset({1, 2, 3, 4})
# Карусели наполнения: ридеры 7 и 8 (меню «Считыватели»).
MIRIADA_FILLING_READERS = frozenset({7, 8})
# Регистрация полного баллона на складе идёт вне партии — статус уходит сразу.
MIRIADA_WAREHOUSE_STATUS_READERS = frozenset({5})
MIRIADA_IMMEDIATE_STATUS_READERS = MIRIADA_FILLING_READERS | MIRIADA_WAREHOUSE_STATUS_READERS
MIRIADA_BALLOON_STATUS_READERS = MIRIADA_BATCH_STATUS_READERS | MIRIADA_IMMEDIATE_STATUS_READERS
...
def _get_ttn_close_handler():
    """
    Возвращает обработчик закрытия ТТН в Мириаде, если приложение ``ttn`` его уже содержит.

    Электронные ТТН переносятся в Лиду отдельным этапом; до этого партия закрывается
    локально, без обращения к Мириаде за ТТН.

    Returns:
        Callable | None: ``ttn.services.close_ttn_in_miriada`` или None.
    """
    try:
        from ttn.services import close_ttn_in_miriada
    except ImportError:
        logger.warning(
            'Закрытие ТТН в Мириаде недоступно: ttn.services.close_ttn_in_miriada не найден'
        )
        return None
    return close_ttn_in_miriada
...
def should_defer_balloon_status_to_batch_close(reader_number: int) -> bool:
    """
    Статусы баллонов рамок отгрузки (1/2) и приёмки (3/4) отправляются в Мириаду
    только при закрытии активной партии, а не в момент сканирования.
    Наполнение (7/8) и склад (5) уходят сразу.
...
def should_send_balloon_status_immediately(reader_number: int) -> bool:
    """
    На лету в Мириаду уходят наполнение (7/8) и регистрация полного баллона
    на складе (5). Рамки партий 1/2/3/4 — при закрытии партии.
    ...
    """
    return reader_number in MIRIADA_IMMEDIATE_STATUS_READERS
...
def attempt_close_balloons_batch(batch: BalloonsBatch) -> Tuple[bool, Optional[str]]:
    ...
    """
    close_ttn_in_miriada = _get_ttn_close_handler()
    ...
    should_send = bool(batch.ttn_id) and close_ttn_in_miriada is not None
    if should_send:
