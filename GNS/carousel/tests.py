...
class LoadCarouselConfigsTests(TestCase):
    def setUp(self):
        # data-migration может сидить number 1..N и ReaderSettings 7/8
        CarouselSettings.objects.all().delete()
        self.reader_8, _ = ReaderSettings.objects.update_or_create(
            number=8,
            defaults={'ip': '10.0.0.8', 'need_cache': True, 'status': 'r8'},
        )
        self.reader_9, _ = ReaderSettings.objects.update_or_create(
            number=9,
            defaults={'ip': '10.0.0.9', 'need_cache': True, 'status': 'r9'},
        )
